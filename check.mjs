...
const problems = [...new Set([...fromHtml, ...fromAgents])].filter(u => !existsExactCase(u)).map(u => `missing (or wrong case): ${u}`);
...
console.log(`ok — ${fromHtml.length} page references, ${AGENTS.length} agents, ${fromAgents.length} media files all present`);
